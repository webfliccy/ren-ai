...
describe("formatIntervalOn", () => {
  ...
  test("counts in days below a week, spelled out", () => {
    const published = new Date("2026-06-25T09:00:00Z");
    expect(formatIntervalOn(published, new Date("2026-06-28T09:00:00Z"))).toBe(
      "three days on",
    );
    expect(formatIntervalOn(published, new Date("2026-06-26T09:00:00Z"))).toBe(
      "one day on",
    );
  });
  ...
  test("counts in months from four weeks, years from twelve months", () => {
    ...
    expect(formatIntervalOn(published, new Date("2026-02-04T09:00:00Z"))).toBe(
      "one month on",
    );
    expect(formatIntervalOn(published, new Date("2026-07-06T09:00:00Z"))).toBe(
      "six months on",
    );
    expect(formatIntervalOn(published, new Date("2027-01-02T09:00:00Z"))).toBe(
      "one year on",
    );
    expect(formatIntervalOn(published, new Date("2028-03-01T09:00:00Z"))).toBe(
      "two years on",
    );
  });
});

...
export function ToolCard({ tool }: { tool: Tool }) {
    ...
    <>
      {tool.category && (
        <div className="flex items-center justify-between bg-ink px-3.5 py-2 text-parchment">
          <span className="font-courier text-[8.5px] tracking-1 opacity-65">{tool.category}</span>
        </div>
      )}
      {tool.illustration && (
        <SanitizedSvg
      ...
      <div className="flex flex-1 flex-col px-4 pt-4 pb-5">
        <h4 className="mb-2 font-cormorant text-[26px] font-semibold leading-[1.05]">{tool.name}</h4>
        {tool.descriptor && <p className="mb-4 text-[13.5px] leading-[1.55] text-ink-light">{tool.descriptor}</p>}
  ...
  const cardClasses = "group hover-lift flex flex-col border-[1.5px] border-ink bg-paper shadow-paper";

  return tool.url ? (
    <a className={`${cardClasses} no-underline`} href={tool.url} target="_blank" rel="noopener noreferrer">
      {inner}
    </a>
  ) : (

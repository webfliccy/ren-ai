...
// property (rather than faking visibility with CSS) is required: Chromium's
// <details> wraps closed content in an internal ::details-content node that
// author CSS can't force-display. A ref callback (rather than an inline
// <script>) is required too: Next.js client-side navigations mount this
// component via React's renderer instead of the browser's HTML parser, and
// scripts inserted that way never execute, so `open` would stay stuck at
// its closed SSR default after every soft navigation.
function applyDesktopOpenDefault(el: HTMLDetailsElement | null) {
  if (el) el.open = window.matchMedia("(min-width: 760px)").matches;
}

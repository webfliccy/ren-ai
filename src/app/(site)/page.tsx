  ...
  const [allPosts, issueTools, latestNotes] = await Promise.all([
    ...
    currentIssue
      ? db
          ...
          .where(
            and(
              eq(tools.issueId, currentIssue.id),
    ...
    currentIssue
      ? db
          ...
          .where(
            and(
              eq(fieldNotes.issueId, currentIssue.id),
              eq(fieldNotes.status, "published"),
            ),
          )
        ...
        <aside className="py-8 pl-10 max-tablet:px-0 max-tablet:py-6">
          ...
          {sidebarPosts.length > 0
            ...
            : PLACEHOLDER_SIDEBAR.map((item) => (
                ...
                >
                  <span className="font-courier text-[11px] font-bold text-accent">
                    {item.num}
                  </span>
                  <h4 className="mt-1 mb-1.5 font-cormorant text-2xl leading-[1.08] font-semibold">
                    <a className="no-underline hover:text-accent" href="#">
                      {item.title}
                    </a>
                  </h4>
        ...
        <div className="grid grid-cols-3 gap-6 max-tablet:grid-cols-1">
          {issueTools.length > 0
            ...
            : PLACEHOLDER_TOOLS.map((p) => (
                <a
                  key={p.title}
                  className="group flex hover-lift flex-col border-[1.5px] border-ink bg-paper no-underline shadow-paper"
                  href="#"
                >
                  <div className="flex items-center justify-between bg-ink px-3.5 py-2 text-parchment">
                    <span className="font-courier text-[9.5px] font-bold tracking-3 uppercase">
                      {p.title}
                    </span>
